...
def main():
    ...
    csv_logger = K.callbacks.CSVLogger(str(n_rounds) + 'R_64_2048_mse_adam.csv', separator=';', append=False)
    ...
    plt.savefig('acc_' + str(n_rounds) + 'R_64_2048_mse_adam.png', format='png')
    ...
    plt.savefig('loss_' + str(n_rounds) + 'R_64_2048_mse_adam.png', format='png')

    eval = model.evaluate(test_x, test_y, verbose=0)
    print("Evaluation: loss = %0.6f custom_accuracy= %0.2f%%\n" \
        % (eval[0], eval[1]*100) )

    model.save('model_full_' + str(n_rounds) +'R_64_relu_adam_2048.h5')
