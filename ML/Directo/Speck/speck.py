...
class SpeckCipher(object):
    ...
    def __init__(self, key, key_size=128, block_size=128, mode='ECB', outfile='dataset_26R_iter_train_test.txt', init=0, counter=0):
    ...
    def encrypt_function(self, upper_word, lower_word):
        ...
        # Guardamos los resultados de cada ronda en el fichero (incluyendo el texto plano)
        for i in range(48):
            self.outfile.write(str((x >> (47 - i)) & 1))
            self.outfile.write(',')

        for i in range(48):
            self.outfile.write(str((y >> (47 - i)) & 1))
            self.outfile.write(',')

        # Run Encryption Steps For Appropriate Number of Rounds
        for j,k in enumerate(self.key_schedule):
        ...
        # Guardamos el texto cifrado
        for i in range(32):
            self.outfile.write(str((x >> (31 - i)) & 1))
            self.outfile.write(',')

        for i in range(32):
            self.outfile.write(str((y >> (31 - i)) & 1))
            if i != 31:
                self.outfile.write(',')
