...
def main():
    ...
    print(n_rounds, "AES with no SR dataset using Keras/TensorFlow ")

    print("Loading data into memory \n")
    train_data = "dataset_noSR_" + str(n_rounds) + "R_train.txt"
    test_data = "dataset_noSR_" + str(n_rounds) + "R_test.txt"
    ...
    csv_logger = K.callbacks.CSVLogger('noSR_' + str(n_rounds) + 'R_2048_mse_adam.csv', separator=';', append=False)
    ...
    plt.savefig('acc_noSR_' + str(n_rounds) + 'R_2048_mse_adam.png', format='png')
    ...
    plt.savefig('loss_noSR_' + str(n_rounds) + 'R_2048_mse_adam.png', format='png')

    eval = model.evaluate(test_x, test_y, verbose=0)
    print("Evaluation: loss = %0.6f custom_accuracy= %0.2f%%\n" \
        % (eval[0], eval[1]*100) )

    model.save('model_full_noSR_' + str(n_rounds) + '_relu_adam_2048.h5')
