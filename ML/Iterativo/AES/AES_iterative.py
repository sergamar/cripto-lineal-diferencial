...
def main():
    ...
    print(n_rounds, "Round Iterative AES dataset using Keras/TensorFlow ")
    ...
    for i in range(n_rounds + 1):
        train_x_array.append(np.loadtxt(train_data, usecols=cols_entrada[(128*i):(128*(i + 1))],
            delimiter=",", skiprows=0, dtype=np.uint8))
        train_y_array.append(np.loadtxt(train_data, usecols=cols_entrada[(128*(i + 1)):(128*(i + 2))],
            delimiter=",", skiprows=0, dtype=np.uint8))

        test_x_array.append(np.loadtxt(test_data, usecols=cols_entrada[(128*i):(128*(i + 1))],
            delimiter=",", skiprows=0, dtype=np.uint8))
        test_y_array.append(np.loadtxt(test_data, usecols=cols_entrada[(128*(i + 1)):(128*(i + 2))],
            delimiter=",", skiprows=0, dtype=np.uint8))
    ...
    for i in range(n_rounds):
        ...
        h = model.fit(train_x_array[i], train_y_array[i], batch_size=3000, epochs=300, verbose=1, validation_data = (test_x_array[i], test_y_array[i]), shuffle=False, callbacks=[csv_logger])
