...
def main():
    n_rounds = 26
    print(n_rounds, "Round Reverse Iterative Speck dataset using Keras/TensorFlow ")
    ...
    for i in range(n_rounds + 1):
        x_array.append(np.loadtxt(data, usecols=cols_entrada[(64*i):(64*(i + 1))],
            delimiter=",", skiprows=0, dtype=np.uint8))
        y_array.append(np.loadtxt(data, usecols=cols_entrada[(64*(i + 1)):(64*(i + 2))],
            delimiter=",", skiprows=0, dtype=np.uint8))
    ...
    for i in range(n_rounds):
        ...
        h = model.fit(train_y_array[n_rounds - 1 - i], train_x_array[n_rounds - 1 - i], batch_size=1000, epochs=200, verbose=1, validation_data = (test_y_array[n_rounds - 1 - i], test_x_array[n_rounds - 1 - i]), shuffle=False, callbacks=[csv_logger])
    ...
    for i in range(n_rounds):
        eval = model_array[i].evaluate(predicts, test_x_array[n_rounds - 1 - i], verbose=0)
        predicts = model_array[i].predict(test_y_array[n_rounds - 1 - i])
